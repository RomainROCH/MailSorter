...
KNOWN_MALWARE_HASHES = set()  # Populated from threat feed
...
class AttachmentHeuristic:
    ...
    def analyze_attachment(
        ...
        if ext_lower in self.suspicious_extensions:
            ...
            result["flags"].append(f"suspicious_extension:{ext_lower}")
        ...
        if ext_lower in {'.exe', '.msi'} and size_bytes > 0 and size_bytes < 10000:
            ...
            result["flags"].append("suspiciously_small_executable")
        
        # Clamp adjustment
        result["score_adjustment"] = max(-0.5, min(0.9, result["score_adjustment"]))
    ...
    def analyze_attachments(self, attachments: List[Dict]) -> Dict:
        ...
        high_risk_count = sum(1 for d in details if d["risk_level"] in ("high", "critical"))
    ...
    def _has_double_extension(self, filename: str) -> bool:
        ...
        doc_extensions = {'.pdf', '.doc', '.docx', '.xls', '.xlsx', '.jpg', '.png', '.txt'}
        exec_extensions = {'.exe', '.scr', '.bat', '.cmd', '.com', '.pif', '.vbs', '.js'}
