...
class PrivacyGuard:
    ...
    def __init__(self, use_presidio: Optional[bool] = None):
        ...
        logger.info(
            f"PrivacyGuard initialized with {'Presidio' if self.use_presidio else 'regex'} mode"
        )
        ...
        try:
            ...
            self.operators = {
                ...
                "IBAN_CODE": OperatorConfig(
                    "replace", {"new_value": "<IBAN_REDACTED>"}
                ),
                ...
                "DATE_TIME": OperatorConfig(
                    "keep"
                ),  # Keep dates, they're usually relevant for classification
    ...
    def get_pii_stats(self, text: str) -> dict:
        ...
        if self.use_presidio:
            try:
                results = self.analyzer.analyze(
                    text=text, entities=self.PRESIDIO_ENTITIES, language="en"
                )
                return {
        ...
        return {"entities_found": count, "entity_types": types, "engine": "regex"}
