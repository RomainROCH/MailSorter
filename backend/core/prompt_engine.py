...
Antworten Sie nur mit JSON.""",
    ...
    def __init__(
        ...
        self._env: Optional[Environment]
        if JINJA2_AVAILABLE:
    ...
    def build_system_prompt(
        self, folders: List[str], language: Optional[str] = None
    ) -> str:
    ...
    def build_user_prompt(
        ...
        return self.render(
            "classify", subject or "", body_snippet, [], language=lang, sender=sender
        )
    ...
    def detect_language(self, text: str, sender: Optional[str] = None) -> str:
        """
    ...
    def _detect_language_impl(self, text: str) -> str:
        ...
        if LANGDETECT_AVAILABLE:
            ...
            if any(
                w in text_lower
                for w in ["bonjour", "merci", "j'ai", "vous", "nous", "très"]
            ):
                return "fr"
            ...
            if any(
                w in text_lower
                for w in ["guten", "danke", "bitte", "sehr", "nicht", "können"]
            ):
                return "de"

            # Spanish indicators
            if any(
                w in text_lower for w in ["hola", "gracias", "buenos", "usted", "muy"]
            ):
                return "es"

        return self.default_language

    def get_template(self, template_name: str, language: Optional[str] = None) -> str:
        """
...
def get_prompt_engine(
    templates_dir: Optional[str] = None, default_language: str = "en"
) -> PromptEngine:
    """
    ...
    if _prompt_engine is None:
        _prompt_engine = PromptEngine(
            templates_dir=templates_dir, default_language=default_language
        )
