...
class CacheEntry:
    ...
    created_at: InitVar[Optional[float]] = None
    hit_count: int = 0
    source: str = "cache"
    ttl: float = 0  # TTL in seconds, 0 = no expiry

    def __post_init__(self, created_at: Optional[float]):
        # Backwards-compatible support for tests passing created_at=
        if created_at is not None:
            self.timestamp = float(created_at)
...
class CacheRule:
    ...
    rule_id: Optional[str] = None  # Optional unique identifier
...
class SmartCache:
    ...
    DEFAULT_RULES = [
        # Invoices
        {
            "pattern": r"\b(invoice|facture|rechnung|fattura)\b",
            "folder": "Invoices",
            "field": "subject",
        },
        {
            "pattern": r"\b(receipt|reçu|quittung)\b",
            "folder": "Invoices",
            "field": "subject",
        },
        # Newsletters
        {
            "pattern": r"\b(unsubscribe|désabonner|abmelden)\b",
            "folder": "Newsletters",
            "field": "body",
        },
        {
            "pattern": r"\b(newsletter|bulletin)\b",
            "folder": "Newsletters",
            "field": "subject",
        },
        # Notifications
        {
            "pattern": r"^(notification|alert|alerte):",
            "folder": "Notifications",
            "field": "subject",
        },
        {
            "pattern": r"noreply@|no-reply@|donotreply@",
            "folder": "Notifications",
            "field": "sender",
        },
        # Social
        {
            "pattern": r"@(facebook|twitter|linkedin|instagram)\.com$",
            "folder": "Social",
            "field": "sender",
        },
        # Shipping
        {
            "pattern": r"\b(tracking|shipment|delivery|livraison|versand)\b",
            "folder": "Shipping",
            "field": "subject",
        },
    ]
    ...
    def check(
        self, subject: str, body: str, sender: str, folders: List[str]
        ...
        for rule in self._rules:
            ...
            if text and rule.pattern.search(text):
                ...
                if rule.folder in folders:
                    logger.debug(
                        f"Rule match: '{rule.pattern.pattern}' → {rule.folder}"
                    )
    ...
    def _check_sender_cache(
        self, sender: str, folders: List[str]
    ) -> Optional[ClassificationResult]:
    ...
    def _check_hash_cache(
        self, subject: str, body: str, folders: List[str]
    ) -> Optional[ClassificationResult]:
    ...
    def store(
        self, subject: str, body: str, sender: str, folder: str, confidence: float
    ) -> None:
        """
        ...
        with self._lock:
            ...
            if sender_key:
                self._sender_cache[sender_key] = CacheEntry(
                    folder=folder, confidence=confidence, timestamp=now
                )
        ...
        with self._lock:
            ...
            return {
                **self._stats,
                ...
                "sender_cache": {"hits": sender_hits, "misses": sender_misses},
            }
    ...
    def add_rule(
        ...
        pattern: str,
        folder: str,
        field: Optional[str] = None,  # Auto-detect based on pattern
        confidence: float = 0.9,
        priority: int = 0,
        ...
        if match_field is not None:
            ...
            if "@" in pattern:
                ...
                actual_field = (
                    "sender"  # Default to sender for backwards compat with tests
                )
    ...
    def check_rules(self, sender: str, subject: Optional[str] = None) -> Optional[Dict]:
        """
    ...
    def cache_by_sender(self, sender: str, folder: str, confidence: float) -> None:
        ...
        with self._lock:
            self._sender_cache[sender_key] = CacheEntry(
                folder=folder, confidence=confidence, timestamp=time.time()
            )
