        ...
        try:
            ...
            if result and result.folder:
                ...
                if verbose:
                    ...
                    print(
                        f"  {status} Sample {i+1}: expected={email['expected']}, "
...
def main():
    ...
    parser.add_argument(
        "--provider", "-p", help="Specific provider to test (default: all available)"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show per-sample results"
    )
    ...
    for provider_name in providers:
        ...
        if results["available"]:
            ...
            if results["samples_tested"] > 0:
                print(
                    f"  Accuracy: {results['accuracy']:.0%} ({results['correct']}/{results['samples_tested']})"
                )
    ...
    ready_providers = [
        r for r in all_results if r["healthy"] and r["samples_tested"] > 0
    ]
    ...
    if not ready_providers:
        ...
        print(
            f"  ✅ Best performing: {best['provider']} ({best['accuracy']:.0%} accuracy)"
        )
        ...
        if ollama:
            if ollama["accuracy"] >= 0.8:
                ...
                print(
                    "  ⚠️ Ollama marginal - consider cloud fallback for low confidence"
                )
            else:
