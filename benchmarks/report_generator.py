...
def generate_markdown_report(report: Dict, output_path: Optional[str] = None) -> str:
    ...
    if report.get("summary"):
        ...
        lines.append(
            f"| Fastest | **{s['fastest']['provider']}** | {s['fastest']['avg_latency_ms']:.0f}ms |"
        )
        lines.append(
    ...
    lines.append("")
    lines.append(
        "| Provider | Accuracy | Avg Latency | P95 Latency | Errors | Est. Cost |"
    )
    lines.append(
        "|----------|----------|-------------|-------------|--------|-----------|"
    )
    ...
    for name, metrics in sorted(
        report.get("providers", {}).items(), key=lambda x: -x[1].get("accuracy", 0)
    ):
    ...
    for name, metrics in report.get("providers", {}).items():
        ...
        lines.append(
            f"| {name} | {avg_tokens:.0f} | ${cost_per_1k:.2f} | ${monthly:.2f} |"
        )
