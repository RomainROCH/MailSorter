...
class BenchmarkRunner:
    ...
    def __init__(
        ...
        self.folders = folders or [
            "Inbox", "Invoices", "Newsletters", "Spam", 
            "Social", "Shipping", "Support"
        ]
    ...
    def _benchmark_provider(self, provider_name: str):
        ...
        for i, email in enumerate(self.emails):
            ...
            if attempt.error:
                ...
                self._update_category_counts(metrics.per_folder, email.expected_folder, attempt.correct)
                self._update_category_counts(metrics.per_difficulty, email.difficulty, attempt.correct)
                self._update_category_counts(metrics.per_language, email.language, attempt.correct)
        ...
        logger.info(f"  Accuracy: {metrics.accuracy:.1%} ({metrics.correct}/{metrics.total_samples})")
    ...
    def _generate_summary(self) -> Dict:
        ...
        fastest = min(
            self.metrics.values(),
            key=lambda m: m.latency_stats()["avg"] if m.latency_stats()["avg"] > 0 else float('inf')
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        # Error rate
        for name, metrics in self.metrics.items():
            error_rate = metrics.errors / metrics.total_samples if metrics.total_samples > 0 else 0
            if error_rate > 0.1:
                recommendations.append(
        ...
        if not recommendations:
            recommendations.append("✅ All providers performing well. No issues detected.")
    ...
    def print_report(self):
        ...
        print(f"🤖 Providers tested: {', '.join(report['metadata']['providers_tested'])}")
        ...
        if report.get("summary"):
            ...
            print("-" * 40)
            s = report["summary"]
            print(f"  🏆 Best Accuracy: {s['best_accuracy']['provider']} ({s['best_accuracy']['accuracy']})")
            print(f"  ⚡ Fastest: {s['fastest']['provider']} ({s['fastest']['avg_latency_ms']:.0f}ms avg)")
            print(f"  💰 Cheapest: {s['cheapest']['provider']} (${s['cheapest']['cost_usd']:.4f})")
        
        # Comparison table
        print("\n" + "-" * 40)
        print("📊 COMPARISON")
        print("-" * 40)
        print(f"{'Provider':<12} {'Accuracy':<10} {'Latency':<10} {'Errors':<8} {'Cost':<12}")
        print("-" * 52)
...
def main():
    ...
    parser.add_argument(
        "--all", "-a",
        action="store_true",
        help="Benchmark all registered providers"
    )
    parser.add_argument(
        "--dataset", "-d",
