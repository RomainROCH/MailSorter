...
def find_migration_path(from_version: str, to_version: str) -> list[tuple[str, str]]:
    ...
    while version_tuple(current) < target_tuple:
        ...
        if next_migration is None:
            ...
            best_match = None
            for (from_v, to_v), _ in MIGRATIONS.items():
                if version_tuple(from_v) <= version_tuple(current) < version_tuple(to_v):
                    if best_match is None or version_tuple(to_v) < version_tuple(best_match[1]):
...
def main():
    ...
    parser.add_argument(
        "--to", dest="to_version",
        help="Target version (default: current app version)"
    )
    parser.add_argument(
