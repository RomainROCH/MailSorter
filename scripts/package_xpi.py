...
def validate_extension() -> list[str]:
    ...
    if (EXTENSION_DIR / "icons").exists():
        icons_found = list((EXTENSION_DIR / "icons").glob("*.svg")) + list(
            (EXTENSION_DIR / "icons").glob("*.png")
        )
        if not icons_found:
...
def main():
    parser = argparse.ArgumentParser(description="Package MailSorter extension as XPI")
    parser.add_argument(
        "--version", "-v", help="Version number (default: read from manifest.json)"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "--no-checksum", action="store_true", help="Skip creating SHA256 checksum file"
    )
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate, don't create XPI"
    )
