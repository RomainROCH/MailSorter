...
class TestRegisterShScript:
    ...
    def test_script_syntax_valid(self, register_sh: Path):
        ...
        result = subprocess.run(
            ["bash", "-n", str(register_sh)], capture_output=True, text=True, timeout=10
        )
...
class TestXPIPackaging:
    ...
    def test_create_xpi_dry_run(self):
        ...
        for f in extension_files:
            if f.is_file():
                should_exclude = any(pat in str(f) for pat in exclude_patterns)
        ...
        manifest_included = any(f.name == "manifest.json" for f in included_files)
        ...
        with zipfile.ZipFile(xpi_path, "r") as zf:
            for name in zf.namelist():
                for pattern in excluded:
                    assert pattern not in name, f"XPI contains excluded: {name}"
...
class TestUpgradePaths:
    ...
    def test_versions_match(self):
        ...
        assert (
            backend_version == extension_version
...
class TestEnvironmentSetup:
    ...
    def test_makefile_has_common_targets(self):
        ...
        for target in common_targets:
            assert (
                f"{target}:" in content or f"{target} :" in content
