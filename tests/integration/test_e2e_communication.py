...
def mock_orchestrator_deps():
    ...
    with patch('backend.core.orchestrator.ProviderFactory') as mock_factory, \
         patch('backend.core.orchestrator.get_smart_cache') as mock_get_cache, \
         patch('backend.core.orchestrator.get_circuit_breaker') as mock_get_breaker, \
         patch('backend.core.orchestrator.get_rate_limiter') as mock_get_limiter, \
         patch('backend.core.orchestrator.get_prompt_engine') as mock_get_prompt, \
         patch('backend.core.orchestrator.get_calibrator') as mock_get_calibrator, \
         patch('backend.core.orchestrator.get_batch_processor') as mock_get_batch, \
         patch('backend.core.orchestrator.get_feedback_loop') as mock_get_feedback, \
         patch('backend.core.orchestrator.check_rate_limit') as mock_check_rate:
...
class TestClassificationE2EFlow:
    ...
    def test_classify_email_full_flow(self, mock_orchestrator_deps):
        ...
        mock_orchestrator_deps["provider"].classify_email.return_value = ClassificationResult(
            folder="Invoices",
            confidence=0.92,
...
class TestBatchProcessingE2E:
    ...
    def test_batch_start(self, mock_orchestrator_deps):
        ...
        batch_message = {
            ...
            "payload": {
                "emails": [
                    {"id": "1", "from": "a@test.com", "subject": "Test 1", "body": "Body 1"},
                    {"id": "2", "from": "b@test.com", "subject": "Test 2", "body": "Body 2"},
                ]
            }
        }
...
class TestErrorHandlingE2E:
    ...
    def test_provider_error_graceful_degradation(self, mock_orchestrator_deps):
        ...
        mock_orchestrator_deps["provider"].classify_email.side_effect = Exception("LLM unavailable")
