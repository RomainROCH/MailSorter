...
class TestOllamaLiveIntegration:
    ...
    def test_handles_empty_body(self, provider: OllamaProvider):
        ...
        result = provider.classify_email(
            subject="Meeting Tomorrow", body="", available_folders=["Inbox", "Work"]
        )
    ...
    def test_latency_acceptable(self, provider: OllamaProvider):
        ...
        result = provider.classify_email(
            subject="Quick Test", body="Just a quick test.", available_folders=["Inbox"]
        )
...
class TestOllamaNetworkFailures:
    ...
    def test_handles_wrong_url(self):
        ...
        provider = OllamaProvider({"base_url": "http://localhost:99999", "timeout": 5})

        assert provider.health_check() is False

    def test_handles_invalid_model(self):
        """Should handle non-existent model."""
        provider = OllamaProvider({"model": "nonexistent-model-12345", "timeout": 30})

        # Health check might pass (Ollama running) but model missing
        result = provider.classify_email(
            subject="Test", body="Test", available_folders=["Inbox"]
        )
    ...
    def test_timeout_handling(self):
        ...
        provider = OllamaProvider({"timeout": 0.001})  # Unreasonably short
...
@pytest.mark.live
@pytest.mark.skipif(
    not check_api_key_available("openai"), reason="OpenAI API key not configured"
)
class TestOpenAILiveIntegration:
    ...
    def provider(self) -> OpenAIProvider:
        ...
        return OpenAIProvider({"model": "gpt-4o-mini", "timeout": 30})
...
class TestOpenAINetworkFailures:
    ...
    def test_invalid_api_key(self):
        ...
        provider = OpenAIProvider({"api_key": "sk-invalid-key-12345", "timeout": 10})
    ...
    def test_handles_rate_limit(self):
        ...
        provider = OpenAIProvider({"api_key": "sk-invalid", "timeout": 0.001})
...
@pytest.mark.live
@pytest.mark.skipif(
    not check_api_key_available("anthropic"), reason="Anthropic API key not configured"
)
class TestAnthropicLiveIntegration:
    ...
    def provider(self) -> AnthropicProvider:
        ...
        return AnthropicProvider({"model": "claude-3-haiku-20240307", "timeout": 30})
...
class TestAnthropicNetworkFailures:
    ...
    def test_invalid_api_key(self):
        """Should handle invalid API key gracefully."""
        provider = AnthropicProvider({"api_key": "sk-ant-invalid-key", "timeout": 10})
...
@pytest.mark.live
@pytest.mark.skipif(
    not check_api_key_available("gemini"), reason="Gemini API key not configured"
)
class TestGeminiLiveIntegration:
    ...
    def provider(self) -> GeminiProvider:
        ...
        return GeminiProvider({"model": "gemini-2.0-flash", "timeout": 30})
...
class TestGeminiNetworkFailures:
    ...
    def test_invalid_api_key(self):
        """Should handle invalid API key gracefully."""
        provider = GeminiProvider({"api_key": "AIza-invalid-key", "timeout": 10})
...
class TestProviderFactoryLive:
    ...
    @pytest.mark.skipif(
        not check_api_key_available("anthropic"),
...
class TestCrossProviderConsistency:
    ...
    def get_available_providers(self) -> list[LLMProvider]:
        ...
        if check_ollama_available():
            providers.append(
                OllamaProvider({"model": get_ollama_model(), "timeout": 60})
            )
...
class TestNetworkFailureHandling:
    ...
    def test_dns_failure_simulation(self):
        ...
        provider = OllamaProvider(
            {"base_url": "http://nonexistent.invalid.domain:11434", "timeout": 5}
        )
