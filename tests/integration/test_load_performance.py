...
from backend.core.smart_cache import SmartCache, get_smart_cache
from backend.core.rate_limiter import RateLimiter, get_rate_limiter
from backend.core.circuit_breaker import CircuitBreaker, get_circuit_breaker
from backend.providers.base import ClassificationResult
from backend.utils.sanitize import sanitize_text, sanitize_email_payload
...
class EmailGenerator:
    ...
    FOLDERS = [
        "Inbox", "Work", "Personal", "Newsletters", 
        "Invoices", "Shopping", "Spam", "Important"
    ]
...
class TestHighVolumeProcessing:
    ...
    def test_process_1000_emails_sequential(self, mock_orchestrator_deps):
        ...
        orchestrator = Orchestrator({"provider": "mock", "folders": EmailGenerator.FOLDERS})
...
class TestMemoryUsage:
    ...
    def test_memory_stable_after_1000_classifications(self, mock_orchestrator_deps):
        ...
        assert growth_mb < CONFIG.max_memory_growth_mb, \
...
class TestCachePerformance:
    ...
    def test_cache_hit_rate_similar_emails(self):
        ...
        for email in emails:
            result = cache.check(email["subject"], email["body"], email["from"], folders)
            if result:
                hits += 1
            else:
                misses += 1
    ...
    def test_cache_performance_1000_lookups(self):
        ...
        for i in range(1000):
            cache.check(f"subject-{i % 500}", f"body-{i % 500}", f"sender{i % 500}@test.com", folders)
...
class TestConcurrentProcessing:
    ...
    def test_concurrent_rate_limiter(self):
        ...
        limiter = RateLimiter({
            "test-provider": 100  # 100 requests per minute
        })
...
class TestCPUPerformance:
    ...
    def test_sanitization_performance(self):
        ...
        texts = [
            f"Email {i} with PII: john{i}@example.com, 555-{i:04d}"
            for i in range(1000)
        ]
...
class TestStressScenarios:
    ...
    def test_rapid_provider_switches(self, mock_orchestrator_deps):
        ...
        for i in range(50):
            orchestrator = Orchestrator({
                "provider": "mock",
                "folders": EmailGenerator.FOLDERS
            })
    ...
    def test_malformed_input_stress(self, mock_orchestrator_deps):
        ...
        valid = {
            "type": "classify",
            "payload": EmailGenerator.generate_email(0)
        }
